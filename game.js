class Example extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.image(
      "red_particle",
      "https://play.rosebud.ai/assets/red.png?x9OX"
    );
    this.load.audio(
      "place_sound",
      "https://play.rosebud.ai/assets/tokenplace.mp3?unIG"
    );
    this.load.audio(
      "collapse_sound",
      "https://play.rosebud.ai/assets/collapse.wav?j1z9"
    );
    ...
    this.load.audio(
      "turn_sound",
      "https://play.rosebud.ai/assets/turn.mp3?pQJW"
    );
    this.load.image("background", "https://play.rosebud.ai/assets/bg.jpg?Gnp4");
    this.load.image(
      "green_particle",
      "https://play.rosebud.ai/assets/green_particle.png?qfzY"
    );
    this.load.image(
      "red_particle",
      "https://play.rosebud.ai/assets/red_particle.png?FrOK"
    );
  }
  create() {
    ...
    this.startX =
      (this.cameras.main.width - this.squareSize * this.gridSize) / 2;
    this.startY =
      (this.cameras.main.height - this.squareSize * this.gridSize) / 2;
  ...
  createScoreText() {
    ...
    this.greenScoreText = this.add.text(10, 40, "green: 0", {
      fontSize: "24px",
      fill: "#006400",
    });
  }
  ...
  updateSquareVisual(row, col) {
    ...
    if (square.owner === "red") {
      if (square.particles === 1) {
        square.particleImage = this.add.image(
          square.image.x,
          square.image.y,
          "r1_png"
        );
        square.particleImage.setDisplaySize(
          this.squareSize * 0.8,
          this.squareSize * 0.8
        );
      ...
      square.text.setText(
        square.particles > 0 ? square.particles.toString() : ""
      );
      square.text.setVisible(true);
  ...
  updateScoreText() {
    ...
    this.greenScoreText.setText(`green: ${this.scores.green}`);
  }
  checkWinCondition() {
    if (this.scores.red >= 10 || this.scores.green >= 10) {
      this.scene.pause();
      const winner = this.scores.red >= 10 ? "Red" : "green";
      const textColor = winner === "Red" ? "#ff0000" : "#006400";
      const message =
        winner === "Red"
          ? "GGs player red you are the best uwu"
          : "GGs player green you are the best uwu";
      this.add
